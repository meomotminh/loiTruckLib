...
{
    modifyRegister(MCP_CANCTRL, 0xFF, 0x08);

    unsigned long endTime = millis() + 10;
    bool modeMatch = false;
    uint8_t newmode;
    while (millis() < endTime) {
        newmode = readRegister(MCP_CANCTRL);
        
        newmode &= 0xFF;
        
        modeMatch = newmode == CANCTRL_REQOP_ONESHOT;

        if (modeMatch) {
            //Serial.println(newmode, BIN);
            break;
        }
    }

    //Serial.println(readRegister(MCP_CANCTRL), BIN);
...
MCP2515::ERROR MCP2515::setFilter(const RXF num, const bool ext, const uint32_t ulData)
{
    ...
    uint8_t tbufdata[4];
    //Serial.println(ulData,HEX);
    prepareId(tbufdata, ext, ulData);
    //Serial.println(*tbufdata,HEX);
    setRegisters(reg, tbufdata, 8);
